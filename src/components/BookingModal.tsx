...
export function BookingModal({ isOpen, onClose, room, roomName }: BookingModalProps) {
  ...
  const [latestBookingEmail, setLatestBookingEmail] = useState<string | null>(null);
  
  const handleBookingComplete = () => {
    // Get the email from the latest booking
    const sentEmails = JSON.parse(localStorage.getItem('sentConfirmationEmails') || '[]');
    if (sentEmails.length > 0) {
      setLatestBookingEmail(sentEmails[sentEmails.length - 1].to);
    }
  ...
  return (
    <>
              ...
              {isBookingComplete ? (
                ...
                >
                  ...
                  <p className="text-gray-300 text-center mb-6 max-w-xs">
                    ...
                    <span className="text-gold font-medium">{latestBookingEmail}</span>.
                  </p>
                  
                  <div className="flex items-center justify-center space-x-2 text-gold mb-6 cursor-pointer hover:text-gold-light" onClick={handleViewEmail}>
                    <Mail className="h-5 w-5" />
                    <span className="text-sm">View confirmation email</span>
                  </div>
      ...
      <EmailPreview isOpen={showEmailPreview} onClose={() => setShowEmailPreview(false)} />
    </>
  );
}
